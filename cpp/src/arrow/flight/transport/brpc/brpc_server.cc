...
// bRPC transport implementation for Arrow Flight

#include "arrow/flight/transport/brpc/brpc_server.h"
...
std::once_flag kBrpcServerTransportInitialized;
}  // namespace

void InitializeFlightBrpcServer() {
  std::call_once(kBrpcServerTransportInitialized, []() {
    auto* registry = flight::internal::GetDefaultTransportRegistry();
    for (const auto& transport : {"brpc", "brpc+tls", "brpc+tcp", "brpc+unix"}) {
